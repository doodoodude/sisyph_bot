...
class SisyphStatePublisher:
    ...
    def __init__(self, nh):
        ...
        self.qp_wm_init_inv = [(np.array([0,0,0,1]),np.array([0,0,0,0])),
                               (np.array([0,0,0,1]),np.array([0,0,0,0]))]       
        self.qp_wr = [(np.array([0,0,0,1]),np.array([0,0,0,0])),
                      (np.array([0,0,0,1]),np.array([0,0,0,0]))]
        ...
        for fid_tf in transforms:
            ...
            if fid_tf.fiducial_id == self.fid_world: # world -> usb_cam
                if not self.world_found[cam_ind]:
                    ...
                    self.qp_cw_inv_init[cam_ind] = qp_inv(tf_qp) # T_cw_inv = T_wc

                    self.tf_wc_msg[cam_ind] = qp_to_tf_msg(self.qp_cw_inv_init[cam_ind], self.tf_wc_msg[cam_ind])
 
            if fid_tf.fiducial_id == self.fid_robot and self.world_found[cam_ind]:   # world -> robot, world->map, map->odom, 
                q_wr, p_wr = qp_mult(self.qp_cw_inv_init[cam_ind], tf_qp)
                ...
                if not self.robot_found[cam_ind]:
                    ...
                    self.qp_wm_init[cam_ind]  = qp_wr
                    self.tf_wm_init_msg[cam_ind] = qp_to_tf_msg(self.qp_wm_init[cam_ind], self.tf_wm_init_msg[cam_ind])  

                    self.qp_wm_init_inv[cam_ind] = qp_inv(self.qp_wm_init[cam_ind])

                else:

                    self.qp_wr[cam_ind] = qp_wr

                    qp_or = qp_mult(self.qp_wm_init_inv[cam_ind], self.qp_wr[cam_ind])

                    if self.robot_found.all() and self.world_found.all():
                        q_pred_err, p_pred_err = qp_relative(self.pred_qp[cam_ind], qp_or)
                        angle_err, pos_err = abs(q_to_euler(q_pred_err)[2]), np.max(np.abs(p_pred_err))
                        is_pred_ok = angle_err<0.01 and pos_err<0.05

                        lost_cam0 = (tf_time-self.tf_last_time[0])>0.4

                        new_pub_tfs_from = 0 if is_pred_ok else 1 if lost_cam0 else -1
                        # if new_pub_tfs_from!=self.pub_tfs_from: print(f"switch cam: {self.pub_tfs_from}-->{new_pub_tfs_from}")
                        self.pub_tfs_from = new_pub_tfs_from
                
                    pred_step = qp_relative(self.qp_or[cam_ind], qp_or)
                    self.pred_qp[cam_ind] = qp_mult(qp_or, pred_step)

                    self.qp_or[cam_ind] = qp_or

                self.tf_or_msg[cam_ind] = qp_to_tf_msg(self.qp_or[cam_ind], self.tf_or_msg[cam_ind])
            ...
            if self.world_found[cam_ind]:
                ...
                if not self.pub_tfs_from==-1:
                    if cam_ind==self.pub_tfs_from:
                        ...
                        self.update_pos_track(self.tf_or_msg[cam_ind].transform)
